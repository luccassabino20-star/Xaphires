...
// Miolo compartilhado dos dois formulários públicos de anamnese - o de
// resposta por link (AnamnesePublicPage.jsx, paciente já cadastrado
// respondendo pelo token de uma ficha) e o de captação (AnamneseCaptacaoPage
// .jsx, link fixo por template pra gente que ainda não é paciente). Os dois
// são o mesmo wizard por dentro; só mudam `carregar`/`enviar`, que cada
// página resolve pro endpoint certo (por token vs. por template).
//
// Fica fora de tudo (sem AuthProvider, sem i18n) - mesmo isolamento do
// painel /admin (ver CLAUDE.md e main.jsx), texto só em português por ser
// ferramenta de uso único e sem seleção de idioma no envio. O servidor já
// devolve `error` em português (não passa por translateError), então os
// erros abaixo mostram err.message direto - mesmo hábito de admin/api.js.
//
...
// nenhuma "section" cai numa etapa única com todas as perguntas juntas.
...
export default function AnamneseFormularioEtapas({ carregar, enviar, tituloPadrao = "Ficha de anamnese" }) {
  ...
  return (
    <div className="sc-pub">
      <div className="sc-pub-card">
        <h1 className="sc-pub-title">{dados?.templateName || tituloPadrao}</h1>
